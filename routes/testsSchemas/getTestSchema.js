...
module.exports = async function(req, res) {
  ...
  let testSchema = await TestSchema.findById(id)
  ...
  let tests = await Test.find({ testSchema: id }).populate("course", "code");
  ...
  for (const test of tests) {
    ...
    attempts.forEach(attempt => {
      ...
      attempt.questions.forEach(question => {
        ...
        const questionIndex = testSchema.questions.findIndex(
          q => String(q._id) === String(question.questionSchema)
        );
        if (questionIndex > -1) {
    ...
    const averageScore =
      totalScore === 0
        ? 0
        : Math.floor((100 * totalScore) / attempts.length) / 100; // average score in this test
  ...
  testSchema.averageScore =
    totalGlobalScore === 0
      ? 0
      : Math.floor((100 * totalGlobalScore) / totalAttempts) / 100;
