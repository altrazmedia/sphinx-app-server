...
async function getTestAttempts(
  ...
  questions,
  getScores,
  getQuestionScores = false
) {
  let attempts = await TestAttempt.find(testAttemptFindObj)
    ...
    attempts.forEach(attempt => {
      ...
      attempt.questions.forEach(question => {
        ...
        const correctOptions = questions
          .find(item => String(item._id) === String(questionSchema))
      ...
      attempt.score =
        Math.floor((100 * 100 * correctAnswers) / attempt.questions.length) /
        100;
...
function isAnswerCorrect(answer, correctOptions) {
  ...
  if (correctOptions.length === answer.length) {
    const validOptionsAnswered = correctOptions.filter(option =>
