    ...
    if (test.status === "ongoing" || test.status === "finished") {
      ...
      attempt.questions.forEach(question => {
        ...
        if (test.status === "finished") {
          ...
          const correctOptions = matchedQuestion.options
      ...
      if (test.status === "finished") {
        ...
        attempt.correctAnswers = attempt.questions.filter(
          question => question.correct
        ).length;
        attempt.score =
          Math.floor(
            (100 * 100 * attempt.correctAnswers) / attempt.questions.length
          ) / 100;
      }
